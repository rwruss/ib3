...
if (flock($unitFile, LOCK_EX)) {  // acquire an exclusive lock
	...
	fseek($unitFile, $nextUnit*400+72);
	...
	fseek($unitFile, $nextUnit*400+48);
	...
	fseek($unitFile, $nextUnit*400+40);
	...
	fseek($unitFile, $nextUnit*400+80);
	...
	fseek($unitFile, ($nextUnit+1)*400);
	// Basic parameters
	fwrite($unitFile, pack("i*", $startLocation[0],$startLocation[1],1,8,$pGameID, $pGameID,1,$postVals[1],0));
	// Secondary information
	fwrite($unitFile, pack("i*", 1, 0, $nextUnit));
	fseek($unitFile, ($nextUnit+1)*400+396);
	fwrite($unitFile, pack("i", 9990));
	
	// Create a base military unit
	fseek($unitFile, ($nextUnit+2)*400);
	// Basic parameters
	fwrite($unitFile, pack("i*", $startLocation[0],$startLocation[1],1,6,$pGameID, $pGameID,1,$postVals[1],0));
	// Secondary information
	fwrite($unitFile, pack("i*", 1, 0, $nextUnit));
	fseek($unitFile, ($nextUnit+2)*400+396);
	fwrite($unitFile, pack("i", 9990));
	
	// Record new units in unit slot
	$unitSlot = startASlot($gameSlot, $gamePath."/gameSlots.slt");
	addDataToSlot($gamePath."/gameSlots.slt", $townSlot, pack("N", $nextUnit), $gameSlot);		
	addDataToSlot($gamePath."/gameSlots.slt", $unitSlot, pack("N", $nextUnit+1), $gameSlot);		
	addDataToSlot($gamePath."/gameSlots.slt", $unitSlot, pack("N", $nextUnit+2), $gameSlot);
	...
	addtoSlotGen($gamePath."/mapSlotFile.slt", $mapSlot, pack("i", $nextUnit), $mapSlotFile, 404);
	//addDataToSlot($gamePath."/mapSlotFile.slt", $mapSlot, pack("i", $nextUnit), $mapSlotFile);
	//addDataToSlot($gamePath."/mapSlotFile.slt", $mapSlot, pack("i", $nextUnit+1), $mapSlotFile);
	//addDataToSlot($gamePath."/mapSlotFile.slt", $mapSlot, pack("i", $nextUnit+2), $mapSlotFile);
